-- Fix: jsonb string::text ikut tanda kutip ("1"), number::text tanpa (2).
-- Pakai operator #>> '{}' yang mengembalikan teks murni untuk kedua kasus.
...
-- Rescore semua hasil yang sudah tersimpan dengan fungsi normalisasi baru
do $$
declare
  v_r record;
  v_q record;
...
begin
  ...
  loop
    v_score := 0;
    v_total := 0;
    for v_q in select id, correct_answer from public.placement_questions
      where test_id = v_r.test_id
    loop
      v_total := v_total + 1;
      v_ans := v_r.answers -> v_q.id::text;
      if v_ans is not null and v_q.correct_answer is not null then
        begin
          v_corrj := case
            when left(trim(v_q.correct_answer), 1) = '['
    ...
    v_pct := (v_score::numeric / greatest(v_total, 1)) * 100;
    v_level := case when v_pct <= 50 then 'basic' when v_pct <= 85 then 'advance' else 'expert' end;
    update public.placement_results
    set score = v_score, provisional_level = v_level
    where id = v_r.id;
  end loop;
end;
$$;