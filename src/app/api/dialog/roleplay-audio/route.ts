import { createServerSupabaseClient } from '@/lib/supabase-server'
import { NextResponse } from 'next/server'
import { getAccessToken } from '@/lib/drive'

export async function GET(req: Request) {
  try {
    const supabase = await createServerSupabaseClient()
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return new NextResponse('Unauthorized', { status: 401 })

    const url = new URL(req.url)
    const submissionId = url.searchParams.get('submissionId')
    const turnIdx = Number(url.searchParams.get('turn') ?? '0')
    if (!submissionId) return new NextResponse('submissionId required', { status: 400 })

    const { data: submission } = await supabase
      .from('dialog_script_submissions')
      .select('user_id, batch_id, turns')
      .eq('id', submissionId)
      .single()
    if (!submission) return new NextResponse('Not found', { status: 404 })

    const turn = ((submission.turns as { drive_file_id?: string | null }[]) ?? [])[turnIdx]
    if (!turn?.drive_file_id) return new NextResponse('No audio file', { status: 404 })

    if (submission.user_id !== user.id) {
      const { data: enrollment } = await supabase
        .from('enrollments')
        .select('course_id')
        .eq('user_id', submission.user_id)
        .eq('batch_id', submission.batch_id)
        .maybeSingle()
      if (enrollment) {
        const { data: teacherRow } = await supabase.from('teachers').select('id').eq('user_id', user.id).maybeSingle()
        let isTeacher = false
        if (teacherRow) {
          const { data: ct } = await supabase.from('course_teachers').select('teacher_id').eq('course_id', enrollment.course_id).eq('teacher_id', teacherRow.id).maybeSingle()
          isTeacher = !!ct
        }
        if (!isTeacher) {
          const { data: isAdmin } = await supabase.rpc('is_admin')
          if (!isAdmin) return new NextResponse('Forbidden', { status: 403 })
        }
      } else {
        const { data: isAdmin } = await supabase.rpc('is_admin')
        if (!isAdmin) return new NextResponse('Forbidden', { status: 403 })
      }
    }

    const { data: tokenRow } = await supabase
      .from('user_drive_tokens')
      .select('encrypted_refresh_token')
      .eq('user_id', submission.user_id)
      .maybeSingle()
    if (!tokenRow) return new NextResponse('Drive not connected', { status: 400 })
    const accessToken = await getAccessToken(tokenRow.encrypted_refresh_token)
    if (!accessToken) return new NextResponse('Token refresh failed', { status: 502 })

    const metaRes = await fetch(
      `https://www.googleapis.com/drive/v3/files/${turn.drive_file_id}?fields=mimeType,name`,
      { headers: { Authorization: `Bearer ${accessToken}` } },
    )
    if (!metaRes.ok) return new NextResponse('File not found on Drive', { status: 404 })
    const meta = (await metaRes.json()) as { mimeType: string; name: string }
    const fileRes = await fetch(
      `https://www.googleapis.com/drive/v3/files/${turn.drive_file_id}?alt=media`,
      { headers: { Authorization: `Bearer ${accessToken}` } },
    )
    if (!fileRes.ok) return new NextResponse('Failed to fetch file', { status: 502 })

    const headers = new Headers()
    headers.set('Content-Type', meta.mimeType)
    headers.set('Content-Disposition', `inline; filename="${meta.name}"`)
    headers.set('Cache-Control', 'public, max-age=3600')
    return new NextResponse(fileRes.body, { status: 200, headers })
  } catch (e) {
    console.error('[dialog/roleplay-audio] error', e)
    return new NextResponse('Internal error', { status: 500 })
  }
}
